...
class Obstacle(Cell):
    ...
    def get_view_state(self, retrying) -> List[Cell]:
        ...
        if self.direction == Direction.NORTH:
            if retrying == False:
                ...
                if is_valid(self.x + 1, self.y + 2 + EXPANDED_CELL * 2):
                    cells.append(Cell(self.x + 1, self.y + 2 + EXPANDED_CELL *
        ...
        # If obstacle is facing east, then robot's cell state must be facing west
            ...
            if retrying == False:
                ...
                if is_valid(self.x + 1 + EXPANDED_CELL * 2, self.y):
                    cells.append(Cell(self.x + 1 + EXPANDED_CELL * 2,
                ...
                if is_valid(self.x + 2 + EXPANDED_CELL * 2, self.y):
                    ...
                    cells.append(Cell(self.x + 2 + EXPANDED_CELL * 2,
                                 self.y, Direction.WEST, self.obstacle_id, 0))

                # Or (x + 4, y + 1)
                if is_valid(self.x + 2 + EXPANDED_CELL * 2, self.y + 1):
                    cells.append(Cell(self.x + 2 + EXPANDED_CELL * 2, self.y +
        ...
        # If obstacle is facing west, then robot's cell state must be facing east
            ...
            #     cells.append(Cell(self.x - EXPANDED_CELL * 2, self.y, Direction.EAST, self.obstacle_id, 0))
...
class Grid:
    ...
    def is_valid_cell_state(self, state: Cell) -> bool:
        # Checks if given state is within bounds
        return self.is_valid_coord(state.x, state.y)

    def get_view_obstacle_positions(self, retrying) -> List[List[Cell]]:
        optimal_positions = []
