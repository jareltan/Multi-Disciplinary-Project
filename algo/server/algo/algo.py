...
class MazeSolver:
    ...
    def get_optimal_order_dp(self, retrying) -> List[Cell]:
        distance = 1e9
        optimal_path = []
    ...
    def get_neighbors(self, x, y, direction):
        ...
        for dx, dy, md in MOVE_DIRECTION:
            if md == direction:  # if the new direction == md
                ...
                # Turning displacement is 3 by 2
                # bigger_change = 3
                # smaller_change = 2
                ...
                if direction == Direction.NORTH and md == Direction.EAST:
                    ...
                    # Check for valid position
                    if self.grid.reachable(x + 3, y + 2, turn = True) and self.grid.reachable(x, y, preTurn = True):
                        # Get safe cost of destination
                        safe_cost = self.get_safe_cost(x + 3, y + 2)
                        neighbors.append((x + 3, y + 2, md, safe_cost + 10))

                    # Check for valid position
                    if self.grid.reachable(x - 2, y - 3, turn = True) and self.grid.reachable(x, y, preTurn = True):
                        # Get safe cost of destination
                        safe_cost = self.get_safe_cost(x - 2, y - 3)
                        neighbors.append((x - 2, y - 3, md, safe_cost + 10))
